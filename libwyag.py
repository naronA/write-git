...
def object_hash(fd, fmt, repo=None):
    ...
    # Choose constructor dependint on
...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        ...
        # You CANNOT declare the argument as dct=OrderedDict() or
        # all call to the functions will endlessly grow the same dict.
...
def repo_create(path):
    """Create a new repository ath path."""

    repo = GitRepository(path, True)

    # First, we make sure the path either doesnt exist or is an empty dir.

    if os.path.exists(repo.worktree):
        if not os.path.isdir(repo.worktree):
            raise Exception('%s is not adirectory!' % path)
...
def object_write(obj, actually_write=True):
    """ ヘッダーが追加された後にハッシュが計算されることに注意してください。"""
    # Serialize object data
    data = obj.serialize()
    # Add header
    result = obj.fmt + b' ' + str(len(data)).encode() + b'\00' + data
    # Compute hash
